...
## 2.3.1 累加一帧内所有chirp的信号并加窗, 外加去除直流分量
``` c
							...
							OUT float * outputPtr) // 输出数据指针
{
    ...
    /* 判断所有的chirp是不是结束, 如果是新 */
	if (chirpInd == 0)
	{
        // nSamplesPerchir是配置文件中的 `numAdcSamples` 这个值是采样的点数(假设这个值是256: 因为一个采样点包含实部虚部, 所以要除以二, 右移1位就是除以2,那就每个点进行递增 )
		for( i = 0; i < ((int32_t)nSamplesPerChirp>> 1); i++)
		{
            ...
            --------------------------------------------------------------------------------
            将16是因为adc采样的数据是16位采样, 所以这里的操作是将adc的采样数据转换为32位float类型,然后按照采样点进行拼凑, 一个采样点由连个数组成(实部虚部,各占一个flot, 所以是连个float)
            这里是连2个采样点
            */
			_amem8_f2(&outputPtr[4*i])		=	_ftof2((float)_ext(_loll(input), 16, 16), (float)_ext(_loll(input), 0, 16)); // 采样点1
			_amem8_f2(&outputPtr[4*i + 2])	=	_ftof2((float)_ext(_hill(input), 16, 16), (float)_ext(_hill(input), 0, 16)); // 采样点2
		}
        /* 如果采样点是奇数 */
